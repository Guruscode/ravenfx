...
export default function HowItWorksSection({ animationDelay }: { animationDelay: string }) {
  ...
  // Ref for the section to detect when it enters the viewport
  const sectionRef = useRef(null)
  const isInView = useInView(sectionRef, { once: true, margin: "-50px" })

  return (
    <section className="py-24 px-6 lg:px-8 relative" ref={sectionRef}>
        ...
        <div className="relative">
          ...
          <div className="space-y-24">
            {steps.map((step, index) => {
              // Ref for each step to detect when it enters the viewport
